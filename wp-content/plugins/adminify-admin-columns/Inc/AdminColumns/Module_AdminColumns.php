<?php
namespace WPAdminify\Modules\AdminColumns\Inc\AdminColumns;

use WPAdminify\Inc\Utils;
use WPAdminify\Modules\AdminColumns\Libs\Helper;
use WPAdminify\Modules\AdminColumns\Inc\AdminColumns\Lib\Inc\Carbon_Admin_Column;
use WPAdminify\Modules\AdminColumns\Inc\AdminColumns\Lib\Adminify_Columns_Manager;
use WPAdminify\Modules\AdminColumns\Inc\AdminColumns\Lib\Inc\Carbon_Admin_Columns_Manager;
...
class Module_AdminColumns {
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			if (Helper::is_plugin_active('adminify/adminify.php') || Helper::is_plugin_active('adminify-pro/adminify.php')) {
				add_action( 'admin_menu', [ $this, 'jltwp_adminify_admin_columns_menu' ], 15 );
			}
	...
	public function jltwp_adminify_admin_columns_menu() {
		...
		add_submenu_page(
			...
			esc_html__( 'Admin Columns by WP Adminify', 'adminify-admin-columns' ),
			esc_html__( 'Admin Columns', 'adminify-admin-columns' ),
	...
	public function jltwp_adminify_admin_columns_enqueue_scripts() {
		...
		if ( 'admin.php' === $pagenow && 'adminify-admin-columns' === $_GET['page'] ) {
			...
			wp_register_script( 'wp-adminify-admin-columns-manifest', ADMINCOLUMNS_ASSETS . 'js/manifest.js', [], ADMINCOLUMNS_VER, true );
			wp_register_script( 'wp-adminify-admin-columns-vendor', ADMINCOLUMNS_ASSETS . 'js/vendor.min.js', [], ADMINCOLUMNS_VER, true );
			wp_register_script( 'wp-adminify-admin-columns', ADMINCOLUMNS_ASSETS . 'js/wp-adminify--admin-columns.min.js',
			[ 'wp-adminify-admin-columns-manifest', 'wp-adminify-admin-columns-vendor', 'jquery', 'jquery-ui-droppable', 'jquery-ui-draggable' ],
			ADMINCOLUMNS_VER, true );

			wp_enqueue_style( 'wp-adminify-admin-columns', ADMINCOLUMNS_ASSETS . 'css/wp-adminify--admin-columns.min.css', [], ADMINCOLUMNS_VER );
