...
class CustomColumns_Pro extends CustomAdminColumns {
	...
	function adminify_admin_column_head( $column ) {
		$column['featured_image'] = __( 'Thumbnail', 'adminify-admin-columns' );
		return $column;
	}
	...
	function adminify_admin_column_move( $columns ) {
		...
		foreach ( $columns as $key => $title ) {
			if ( $key === 'title' ) {
				$new['featured_image'] = __( 'Thumbnail', 'adminify-admin-columns' );
			}
			$new[ $key ] = $title;
		}
		return $new;
		...
		if ( $column_name === 'featured_image' ) {
			...
			if ( $post_featured_image ) {
				...
				if ( ! empty( $this->options['post_page_column_thumb_image'] ) && is_array( $this->options['post_page_column_thumb_image'] ) && ! empty( $this->options['post_page_column_thumb_image']['url'] ) ) {
					...
					$image_url = ADMINCOLUMNS_ASSETS . 'images/no-thumb.svg';
				}
				echo '<img style="width:55px;height:55px" src="' . esc_url( $image_url ) . '" alt="' . esc_html__( 'No Thumbnail', 'adminify-admin-columns' ) . '"/>';
			}
		}
	}
	...
	function adminify_post_page_id_column_head( $defaults ) {
		$defaults['adminify_post_id'] = esc_html__( 'ID', 'adminify-admin-columns' );
		return $defaults;
	}
