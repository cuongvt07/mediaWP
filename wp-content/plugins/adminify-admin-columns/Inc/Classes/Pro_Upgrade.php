...
class Pro_Upgrade {
	...
	public function dashboard_widget() {
		wp_add_dashboard_widget(
			'jlt_admin_columns_dashboard_widget',                          // Widget slug.
			esc_html__( 'Adminify Admin Columns News & Updates', 'adminify-admin-columns' ), // Title.
		...
		$example_widget_backup = array( 'jlt_admin_columns_dashboard_widget' => $default_dashboard['jlt_admin_columns_dashboard_widget'] );
		unset( $default_dashboard['jlt_admin_columns_dashboard_widget'] );
	...
	public function dashboard_widget_render() {
		...
		echo '<div class="adminify-admin-columns-rss-widget">';

		if ( wp_validate_boolean( $this->get_content( 'is_campaign' ) ) ) { ?>

			<div class="adminify-admin-columns-dashboard-promo" style="--adminify-admin-columns-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				<a target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
					<img src="<?php echo esc_url( $this->get_content( 'image_url' ) ); ?>" alt="Adminify Admin Columns Promo Image" style="width: 100%; height: auto;">
				</a>

				<a class="adminify-admin-columns-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>">
		...
		if ( ! empty( $maxitems ) ) {
			...
			foreach ( $rss_items as $item ) {
				...
				// $item_date = human_time_diff( $item->get_date( 'U' ), current_time( 'timestamp' ) ) . ' ' . esc_html__( 'ago', 'adminify-admin-columns' );
		...
		<div class="adminify-admin-columns-dashboard_footer">
			<ul>
				<li class="adminify-admin-columns-overview__blog">
                    <a href="https://jeweltheme.com/blog" target="_blank">
                        Blog
                        <span class="screen-reader-text">
                            <?php echo esc_html__( '(opens in a new window)', 'adminify-admin-columns' ); ?>
                        </span>
                        <span aria-hidden="true" class="dashicons dashicons-external"></span>
                    </a>
                </li>
                <li class="adminify-admin-columns-overview__help">
                ...
                <li class="adminify-admin-columns-overview__upgrade">
		...
		<style>
			...
			.adminify-admin-columns-rss-widget .hndle.ui-sortable-handle img {
				margin: -5px 10px -5px 0;
			}

			.adminify-admin-columns-rss-widget .adminify-admin-columns-dashboard_footer {
				margin: 0 -12px -12px;
				padding: 0 12px;
				border-top: 1px solid #eee;
			}

			.adminify-admin-columns-rss-widget .adminify-admin-columns-dashboard_footer ul {
				display: flex;
				list-style: none;
			}

			.adminify-admin-columns-rss-widget .adminify-admin-columns-dashboard_footer ul li:first-child {
				padding-left: 0;
				border: none;
			}

			.adminify-admin-columns-rss-widget .adminify-admin-columns-dashboard_footer li {
				padding: 0 10px;
				margin: 0;
				border-left: 1px solid #ddd;
			}

			.adminify-admin-columns-rss-widget .adminify-admin-columns-overview__go-pro a {
				color: #FCB92C;
				font-weight: 500;
			}
		</style>
	...
	public static function get_data() {
		return get_option( 'jlt_admin_columns_sheet_promo_data' );
	}
	...
	public function get_data_hash() {
		return get_option( 'jlt_admin_columns_sheet_promo_data_hash' );
	}
	...
	public function register_sync_hook() {
		$hook_action = 'jlt_admin_columns_sheet_promo_data_remote_sync';
		...
		register_deactivation_hook( ADMINCOLUMNS_FILE, array( $this, 'clear_register_sync_hook' ) );
	}
	...
	public function clear_register_sync_hook() {
		wp_clear_scheduled_hook( 'jlt_admin_columns_sheet_promo_data_remote_sync' );
	}
	...
	public function sheet_data_remote_sync() {
		...
		if ( $force || $sheet_hash_data !== $sheet_data_hash ) {
			update_option( 'jlt_admin_columns_sheet_promo_data', $remote_data );
			update_option( 'jlt_admin_columns_sheet_promo_data_hash', $sheet_data_hash );
			do_action( 'jlt_admin_columns_sheet_promo_data_reset' );
		}
	}
...
", $response));
		...
		$plugin_slug = Helper::jlt_admin_columns_slug_cleanup();
	...
	public function display_popup() {
		...
		<div class="adminify-admin-columns-popup adminify-admin-columns-upgrade-popup" id="adminify-admin-columns-popup" data-plugin="<?php echo esc_attr( $this->slug ); ?>" tabindex="1" style="display: none;">

			<div class="adminify-admin-columns-popup-overlay"></div>

			<div class="adminify-admin-columns-popup-modal" style="background-image: url('<?php echo esc_url( $image_url ); ?>'); --adminify-admin-columns-popup-color: <?php echo esc_attr( $this->get_content( 'btn_color' ) ); ?>;">
				...
				<div class="adminify-admin-columns-popup-modal-footer">
					...
					<div class="adminify-admin-columns-popup-countdown" style="display: none;">
						<?php if(!empty( $this->get_content( 'notice' ) )){ ?>
							<span data-counter="notice" style="color:#F4B740; font-size:14px; padding-bottom:20px; font-style:italic;">
								<?php echo esc_html__( 'Notice:', 'adminify-admin-columns' );?> <?php echo $this->get_content( 'notice' );?>
							</span>
						<?php } ?>
						<span class="adminify-admin-columns-popup-countdown-text"><?php echo esc_html__( 'Offer Ends In', 'adminify-admin-columns' );?></span>
					...
					<!-- button  -->
					<a class="adminify-admin-columns-popup-button" target="_blank" href="<?php echo esc_url( $this->get_content( 'button_url' ) ); ?>"><?php echo esc_html( $this->get_content( 'button_text' ) ); ?></a>
				</div>
			</div>
		</div>

		<script>
			var $container = jQuery('#adminify-admin-columns-popup'),
				plugin_data = <?php echo json_encode( $this->get_sheet_promo_remote_data(), true ); ?>,
				events = {}; //Events

			// Update Counter
			function updateCounter(seconds) {
				const $counter = $container.find(".adminify-admin-columns-popup-countdown-time");
			...
			function initCounter(last_date) {
				$container.find(".adminify-admin-columns-popup-countdown-time").show();
				...
				if (result) {
					...
					$container.find(".adminify-admin-columns-popup-countdown").show(0);
				} else {
					trigger("countdownFinish", [plugin_data]);
					$container.find(".adminify-admin-columns-popup-countdown").hide(0);
				}
